...
- 1.1 - Retornar a quantidade de estudantes presentes para uma entrada específica;

- 1.2 - Retornar `None` se em `permanence_period` houver alguma entrada inválida;

- 1.3 - Retornar `None` se  `target_time` recebe um valor vazio;

- 1.4 - A função deve, por meio de análise empírica, se comportar (no avaliador remoto em sua Pull Request) como no máximo O(n), ou seja, com complexidade assintótica linear.
...
- O código deve foi feito dentro do arquivo `challenges/challenge_palindromes_recursive.py`.
...
- 2.1 - Retornar `True` se a palavra passada por parâmetro for um palíndromo;

- 2.2 - Retorar `False` se a palavra passada por parâmetro não for um palíndromo;

- 2.3 - Retorar `False` se nenhuma palavra for passada por parâmetro.
